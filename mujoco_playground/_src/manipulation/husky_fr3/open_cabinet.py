  ...
  return config_dict.create(
      ...
      reward_config=config_dict.create(
          scales=config_dict.create(
              ...
              no_barrier_collision=0.1,
              # Do not self-collide.
              no_self_collision=0.25,
              # Arm stays close to target pose.
              robot_target_qpos=0.3,
            #   # Base movement discouraged near goal.
            #   base_motion_modulated=0.2,
          )
      ),
  )


class HuskyFR3OpenCabinet(base.HuskyFR3Base):
  ...
  def reset(self, rng: jax.Array) -> State:
    ...
    target_pos = jp.array([1.3, 0.0, 0.5])
  ...
  def _get_rewards(self, data: mjx.Data, info: dict):
    ...
    # diff_xy  = (box_pos - gripper_pos)[:2]                       # project to xy
    # dist_g2h = jp.linalg.norm(diff_xy) 

    # wheel_vel     = data.qvel[self._robot_base_qposadr] 

    # near_thresh   = 0.20
    # far_thresh    = 0.05
    # modulator = jp.clip((near_thresh - dist_g2h) / (near_thresh - far_thresh), 0.0, 1.0)
    # base_motion_modulated = -jp.mean(jp.square(wheel_vel)) * modulator
    ...
    return {
        ...
        # "base_motion_modulated": base_motion_modulated,
    }
