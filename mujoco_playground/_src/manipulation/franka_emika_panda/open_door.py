...
class PandaOpenDoor(panda.PandaBase):
  ...
  def reset(self, rng: jax.Array) -> State:
    ...
    # target_pos = jp.array([0.3, 0.35, 0.5])
    target_pos = jp.array([-0.05, -0.4, 0.5])
  ...
  def calc_ctrl(self, prev_ctrl: jax.Array, action: jax.Array) -> jax.Array:
    delta = action * self._config.action_scale
    ctrl = prev_ctrl + delta
    ctrl = jp.clip(ctrl, self._lowers, self._uppers)
    return ctrl

  def step(self, state: State, action: jax.Array) -> State:
    """Advances the environment by one timestep."""
    ctrl = self.calc_ctrl(state.data.ctrl, action)
